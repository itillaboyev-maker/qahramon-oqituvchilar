...
**Status:** Active. Any future feature (Community Score, Hero Discovery, etc.) must
preserve this — internal signal, human decides, never a public leaderboard.
...
re-confirmation, not silent adoption.
**Status:** Active. Revisit only with an explicit, scoped request — not a wholesale
codebase merge.
...
**Status:** Active. Next phase: add `pgvector` + embedding similarity as an additional
